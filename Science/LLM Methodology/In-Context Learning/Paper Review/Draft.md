...
The prompt employs hierarchical modular structure, building upon hierarchical decomposition and chain-of-thought techniques. The instructions structure is formatted using Markdown, which is submitted directly to the model; this formatting is crucial for helping the model parse and understand the intended structure and relationships between the steps. The core of the prompt's methodology lies in **Section [IV. Specific Analysis Instructions (Baseline Framework)][Framework]**, which implements the simulated peer review workflow.
...
This method utilizes a form of in-context learning. While conceptually similar to persistent instructions in features like Custom GPTs or Gemini Gems, this implementation activates the instructions directly via the initial chat prompt. A more important difference is that the core function of the prompt is to systematically define workflows for highly complex tasks, it is essentially a program written in natural language.
