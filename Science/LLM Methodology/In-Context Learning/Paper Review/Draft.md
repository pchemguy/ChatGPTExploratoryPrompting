...
A proof-of-concept prompt presented in this report demonstrates the ability of frontier reasoning large language models (LLMs) to perform a meaningful systematic multimodal in-depth analysis of experimental chemistry manuscripts. This analysis is a complex and challenging task for LLMs, partly because of generally closed nature of the scientific peer review process. The proposed approach to compensate for training data deficiency dubbed Persistent Workflow Prompting (PWP), relies solely on in-context learning and a combination of advanced prompt engineering techniques; the entire LLM interaction is performed via the standard prompt, and no API access or additional tools are used. Furthermore, a detailed analysis is elicited via one master prompt. This prompt exhibits a modular Markdown-formatted architecture that defines desired analyses as workflows composed of specific simpler tasks "understood" by the AI. The prompt is not designed to elicit a one-off answer. Instead, it is submitted as the first message in a new LLM chat or session and instructs LLM to apply relevant definition blocks to subsequent queries. The prompt features hierarchical descriptions of analysis workflows (e.g., analysis of figures) and definitions of relevant concepts (e.g., core experimental protocol). Subsequent queries involving such concepts (e.g., "Provide a detailed description of each figure associated with the core experimental protocol") trigger associated multistep workflows yielding the final result. Among other things, the prompt identifies experimental issues and demonstrates the ability to drive inference of missing information by performing multimodal analysis of photographs and integrating extracted information with other parts of manuscript, baseline model knowledge, and information accessible via the Internet. Extracted and inferred information is then used for basic modeling of the key processes, estimating key parameters, comparing estimations with claimed results, and concluding on the feasibility of the main claim.
