...
Sub ProcessMarkup()
    ...
    ' --- Loop 1: Find LNK_PATTERN (Backward), remove Hyperlinks ---
    ...
    ' --- Loop 2: Find BMK_PATTERN (Backward), remove Bookmarks ---
    ...
    ' --- Loop 3: Find BMK_PATTERN (Backward), Call Helper Sub ---
    ...
    With findRange.Find
        ...
        Do While .Execute = True
            ...
            ' Call the helper subroutine, passing the found range.
            ProcessBMK findRange
            findRange.Collapse wdCollapseEnd
    ...
    ' --- Loop 4: Find LNK_PATTERN (Backward), Call ProcessLNK to create Hyperlinks ---
...
' and adds bookmark if valid.
' =========================================================================
Sub ProcessBMK(matchedRange As Range)
    ...
    strippedText = Left$(rawText, Len(rawText) - 2)

    ' --- 3. Split the string at }}{{ ---
    Dim potentialName As String
    ' Find the starting position of the delimiter using InStr
    Dim delimiterPos As Long ' To store position of delimiter
    Const DELIMITER As String = "}}{{BMK:" ' Define delimiter as constant
    delimiterPos = InStr(1, strippedText, DELIMITER, vbBinaryCompare) ' Case-sensitive search is fine for fixed delimiter
    potentialName = Trim$(Mid$(strippedText, delimiterPos + Len(DELIMITER))) ' Extract the substring starting *after* the delimiter
    ...
    If Len(potentialName) = 0 Then
        isValid = False
        Debug.Print "  -> Validation Failed: Name is empty."
    End If

    ' Check length (max 40 chars for Word bookmarks) - Still need this check
    If Len(potentialName) > 40 Then
        isValid = False
        Debug.Print "  -> Validation Failed: Name exceeds 40 characters ('" & potentialName & "')."
    End If

    ' Check structure using Regular Expression (Starts with letter, contains only A-Z, a-z, 0-9, _)
    Dim regex As RegExp
    If isValid Then
        Set regex = New RegExp
        With regex
            ' Pattern: ^ = start, [A-Za-z] = first char letter, [A-Za-z0-9_]* = zero or more allowed chars, $ = end
            .Pattern = "^[A-Za-z][A-Za-z0-9_]*$"
            .Global = False ' Only need to test the whole string once
            .IgnoreCase = False ' Pattern handles case sensitivity
        End With

        ' Perform the test
        If Not regex.Test(potentialName) Then
            isValid = False
            Debug.Print "  -> Validation Failed: Name does not match required pattern (Starts with letter, contains only Alphanumeric or Underscore) ('" & potentialName & "')."
        End If
        ' Note: Cleanup (Set regex = Nothing) happens in Exit/Error handling paths
    End If

    ' --- 7. Create Bookmark if all checks passed ---
    If isValid Then
        Debug.Print "  -> Validation Passed for name: '" & potentialName & "'"
        On Error GoTo ProcessBMK_Error ' Turn error handling back on properly
        ...
        Else
             ...
             ' Optionally reset Err object: Err.Clear
        End If
    Else
...
ProcessBMK_Error:
    ...
    Debug.Print "  -> An error occurred in ProcessBMK!" & vbCrLf & _
                "     Error Number: " & Err.Number & vbCrLf & _
...
' Extracts potential bookmark name from range text, validates it,
' checks if bookmark exists, and adds hyperlink if valid and exists.
' =========================================================================
Sub ProcessLNK(matchedRange As Range)
    ...
    ' --- 2. Strip last two characters ---
    Dim strippedText As String
    If Len(rawText) >= 2 Then
        strippedText = Left$(rawText, Len(rawText) - 2)
    Else
        strippedText = ""
        Debug.Print "  -> Text too short to strip last two chars."
    End If
    ...
    If Len(strippedText) > 0 Then
        ...
        If delimiterPos > 0 Then
            potentialBookmarkName = Mid$(strippedText, delimiterPos + Len(DELIMITER))
            potentialBookmarkName = Trim$(potentialBookmarkName) ' Trim result
            Debug.Print "  -> Potential Target Bookmark Name: '" & potentialBookmarkName & "'"
        Else
            Debug.Print "  -> Delimiter '" & DELIMITER & "' not found after stripping."
        End If
    End If
    ...
    If isNameValid And bookmarkExists Then
        ...
        ' Add the hyperlink. Anchor is the range found by LNK_PATTERN search.
        ' SubAddress is the name of the bookmark to link to.
