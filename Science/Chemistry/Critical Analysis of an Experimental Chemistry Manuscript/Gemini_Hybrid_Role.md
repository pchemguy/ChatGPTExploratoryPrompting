    ...
    **(Repeat the following subsection structure for EACH Core Stage identified in D.2.2)**
    - **Stage {Identifier}. {Stage Name}:** (e.g., **Stage A. Synthesis of XYZ**)
        ...
        - **F. Idealized Model Performance Estimation (Stage-Level):**
            ...
            3. **Calculation:** Using the idealized model, relevant parameters derived from the manuscript description for this stage (e.g., extent of reaction, temperature range, concentration changes), and any sourced literature values, perform a back-of-the-envelope or order-of-magnitude calculation. Estimate the **theoretical maximum performance** achievable by this stage under *idealized conditions* (e.g., theoretical yield limit, maximum achievable purity). Make sure to calculate value that is directly comparable to the associated key reported metrics the final calculated value must be of the same kind and have the same units (e.g., if the key characteristic is reported as absolute value, absolute change, or relative change, the final calculated number should be of the same kind for direct comparison; if the key characteristic is reported as percentage or fraction, the final calculated value would need to have the same representation). Show the key equation(s) used and the calculation steps in detail.
